...
/**
 * MODIFIED FOR JDIFF. I would have simply extended it, but functions rely on properties that have private access...
 * so sad.
 */
...
class WebDriver
{
    ...
    /**
     * Author: Marc Weiner
     * Takes screenshot of current screen, saves it to specified file.
     * @param String $file_loc
     * @throws \Exception
     * @return boolean
     */
    public function screenshot_put($file_loc)
    {
        $command = "screenshot";
        $urlHubFormatted = $this->_hubUrl . "/session/{$this->_sessionId}/{$command}";
        ...
        if (isset($results["value"]) && trim($results["value"]) != ""){
            file_put_contents($file_loc, base64_decode($results["value"]));
            return true;
        }
